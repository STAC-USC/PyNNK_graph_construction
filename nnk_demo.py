...
flags.DEFINE_string('metric', 'rbf', 'Similarity metric to use for finding neighbors')
...
def main(argv=None):
    ...
    utils.plot_graph(W_knn, X, filename=os.path.join(model_output_folder, "KNN"), vertex_color=y)
    W_nnk = nnk_graph(G, knn_mask, FLAGS.knn_param, FLAGS.thresh)
    utils.plot_graph(W_nnk, X, filename=os.path.join(model_output_folder, "NNK"), vertex_color=y)
